...
class Actor(object):
    ...
    def build_train(self,learning_rate = 0.0001):
        with tf.variable_scope(self.name) as scope:
            ...
            train_op = tf.train.GradientDescentOptimizer(learning_rate).apply_gradients(zip(var_grads,self.trainable_vars))
...
class AsyncNets(object):
    def __init__(self,n_observation, n_action,class_name):
        class_ = eval(class_name)
        self.net = class_(n_observation,n_action,name=class_name)
        self.target_net = class_(n_observation,n_action,name='{}_target'.format(class_name))
...
def UONoise(action_n):
    theta = 0.8
    sigma = 0.15
    state = sigma*np.random.randn(action_n)
    while True:
        yield state
        state = sigma*np.random.randn(action_n)
